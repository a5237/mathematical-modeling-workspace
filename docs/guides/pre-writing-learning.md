...
2. 结构、摘要组织、篇幅分配与表达策略的学习必须使用**同赛事**范文：CUMCM 项目用 `resources/paper-library/00-format-layout/cumcm/`，MCM/ICM 项目用 `resources/paper-library/00-format-layout/mcm-icm/` 或经核验的公开获奖论文；跨赛事范文只可用于数学方法与论证逻辑学习，不得计入结构学习的范文数量。
3. 分别提取摘要组织、问题数学化、模型建立、求解说明、结果分析、验证和图表叙事的逻辑。
