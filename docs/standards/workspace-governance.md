...
`00-admin/artifact-map.yaml` 是跨阶段快速导航入口，不是新的权威来源。进入论文写作、结果分析、制图或审校时，先按当前子问题从地图定位稳定关键产物，再到运行手册、模型记录和证据台账核验语义与状态。地图只随下游会复用的关键文件变化而更新，不登记临时文件、全部辅助代码、哈希或阶段状态；旧项目缺失或条目失效时做相关目录内的定向搜索并修复实际用到的路径，不因此扩张为全项目盘点或新增发布门禁。

上游产物发生实质变化时，使用 `tools/trace-artifact-impact.py` 从显式变化路径计算传递影响。`STALE` 只用于标出必须重新生成的派生结果、验证和论文图表副本，`RECHECK` 用于标出应结合新结果复核的证据、正文、审校与交付；核对证明未受影响的内容可以复用。工具输出是影响分析起点，不是持久状态、审校结论或独立门禁，具体复查范围仍执行 `docs/standards/paper-quality-audit.md`。
...
- `test/` 可在模型选择前用于低成本候选比较，也可在正式模型建立后用于参数、算法或局部实现的对照试验。它不形成新的阶段、完成状态或进入下一阶段的前置条件。
- 实验区默认不具备权威性，不进入产物地图、证据台账、论文引用和交付。只有将已采纳方案重新落实到编号阶段、完成正式运行与验证后，才按正式路径触发影响分析和后续复核。
- 对尚未采纳的实验区改动，不要求重跑正式项目；不得反过来用一次低成本实验替代正式阶段既有的模型选择、复现、验证、证据、写作或发布门禁。
