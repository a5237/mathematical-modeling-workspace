...
- `major`：摘要与正文数值冲突；关键约束、单位、验证或参数缺失；每问分析不足两类；模型选择记录未完成算法资源库匹配或库外偏离说明；写作前学习记录缺失、少于 2 篇同类优秀论文或仅有空洞完成声明；关键文献未核验；交付材料不完整；最终 PDF 中存在影响文字识别、连接关系或判断逻辑的图文重叠、裁切、模糊或流程歧义。
