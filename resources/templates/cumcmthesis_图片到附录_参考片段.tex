% !TEX program = xelatex
% CUMCM 2026 论文生成模板（电子版）
% 使用说明：
% 1. 建议将本文件复制到标准项目的 06-paper/ 后再填写。
% 2. 正式提交前必须替换全部“【……】”占位符，删除未使用的示例块，并逐页检查最终 PDF。
% 3. 电子版论文不包含承诺书和编号专用页；纸质版所需专用页按赛区要求另行处理。
% 4. 子问题数量、图表、公式和附录按赛题实际增减，不得机械保留四问示例。
% 5. 论文中的具体数值、图表、结论、引用和检验必须来自已保存且可复现的真实产物。
...
% ============================================================
% 摘要专用页：标题、摘要正文和关键词共同独占本页。
% 摘要应逐问写明任务、模型、求解、关键结果和可信度；不得引用图、表、公式或文献。
% 核心模型、关键数值、最终方案和结论性短语必须加粗。
% ============================================================
\thispagestyle{fancy}
\begin{center}
  {\bfseries\zihao{-2}\PaperTitle\par}
  \vspace{1.2em}
...
% ============================================================
% 正文：正式提交稿不设目录；正文不超过 30 页，附录不计入该上限。
% ============================================================
\section{问题重述}
...
\subsubsection{问题一模型的分析}
\TemplateField{用三至四行完成本问就近数学化，说明模型输入、输出和选择理由。}
...
\TemplateField{说明继承问题一的内容及本问新增的目标、约束、参数和输出。}
...
\subsubsection{问题三模型的分析}
\TemplateField{说明情景变化、继承关系和本问模型调整。}
...
\subsubsection{问题四模型的分析}
\TemplateField{将开放任务转化为明确的评价、预测、优化、仿真或决策任务。}
...
% 使用 AI 时，支撑材料中必须同时提交“AI工具使用详情.pdf”。
% ============================================================
\clearpage
\section*{AI工具使用声明}
\ifusedai
本参赛队在竞赛过程中使用了AI工具，主要用于\AIUsageSummary，详细使用情况见支撑材料。
\else
本参赛队在竞赛过程中未使用任何AI工具。
\fi

% 参考文献原则上不少于 6 篇；所有条目必须真实核验并在正文实际引用。
% 除两本指定教材外，至少 4 篇应与题目背景、模型、算法、参数或验证直接相关。
...
\begin{longtable}{>{\centering\arraybackslash}p{0.08\textwidth}p{0.34\textwidth}p{0.46\textwidth}}
  ...
  \ifusedai
  4 & \texttt{AI工具使用详情.pdf} & 2026 年规定要求的 AI 工具使用详情说明 \\
  \fi
