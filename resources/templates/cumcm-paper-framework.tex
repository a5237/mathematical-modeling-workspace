...
% 结构与内容执行 paper-writing.md，纯排版执行 PW-FMT-001，年度规则执行 OFFICIAL-CUMCM-001，证据执行 WG-EVID-001。
...
% 字体选择与最终 PDF 字体检查只执行 PW-FMT-001。
...
% 小四正文按约 20 pt 基线距排印，对应排版规范的紧凑 1.5 倍视觉节奏。
\linespread{1.38}
...
% AI 声明状态与内容只执行 WG-AI-001 和 OFFICIAL-CUMCM-001。
\newif\ifusedai
\usedaitrue
\newcommand{\AIUsageSummary}{语言润色、代码调试与排版核验等}

% 程序与支撑材料状态只执行 WG-RELEASE-001 和 OFFICIAL-CUMCM-001。
\newif\ifusedprogram
\usedprogramtrue
  ...
  \IfFileExists{#3}{%
    ...
    \fbox{%
      \begin{minipage}{0.94\linewidth}
        \textbf{代码缺失：}\texttt{\detokenize{#3}}（执行 OFFICIAL-CUMCM-001）。
      \end{minipage}%
    }
    \par
  }%
...
% ============================================================
% 摘要内容执行 paper-writing.md，版式执行 paper-formatting.md 和 OFFICIAL-CUMCM-001。
% ============================================================
\thispagestyle{fancy}
\begin{center}
...
\indent\TemplateField{用两至三句话概括研究对象、总体建模主线和主要成果。}
...
% ============================================================
% 正文内容执行 paper-writing.md，目录、页面与版式执行 paper-formatting.md 和 OFFICIAL-CUMCM-001。
% ============================================================
\section{问题重述}

\subsection{问题背景}
\TemplateField{用自己的语言概括背景、研究对象和应用意义，不照抄赛题，不提前展开模型和结果。}
\TemplateField{按 PW-CITE-001 与 WG-EVID-001 写入背景事实和相邻引用，例如 \cite{topic-ref-1,topic-ref-2}。}
...
\begin{figure}[htbp]
  \centering
  \safeincludegraphics[width=0.72\textwidth]{figures/overall-flow.png}
  \caption{总体建模流程}
  \label{fig:overall-flow}
\end{figure}
\TemplateField{按 PW-FIG-001 解释该图与本问论证的关系。}

\section{模型假设}
\begin{enumerate}[label=\textbf{假设\arabic*.}]
  \item \TemplateField{必要假设及其物理、业务或统计依据。}
  \item \TemplateField{按论文写作规范填写必要假设及其合理性。}
...
\end{aligned}
\end{equation}
\cref{eq:q1-model}中，$f(\symbf{x})$表示\TemplateField{目标的实际含义}，$g_1$、$g_2$和$h_1$分别描述\TemplateField{资源、业务或机理约束}，$\Omega$为\TemplateField{变量定义域}。\TemplateField{按实际模型替换示例。}
...
\TemplateField{按 PW-VAL-001 呈现本问的验证证据与判断。}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{0.31\textwidth}
    \centering
    \safeincludegraphics[width=0.95\linewidth]{figures/q2-case-a.png}
    \caption{情景一}
    \label{fig:q2-case-a}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.31\textwidth}
    \centering
    \safeincludegraphics[width=0.95\linewidth]{figures/q2-case-b.png}
    \caption{情景二}
    \label{fig:q2-case-b}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.31\textwidth}
    \centering
    \safeincludegraphics[width=0.95\linewidth]{figures/q2-case-c.png}
    \caption{情景三}
    \label{fig:q2-case-c}
  \end{subfigure}
...
先直接回答本问：\textbf{【填写问题二最终结果】}。\TemplateField{按 PW-VAL-001 呈现本问验证。}
...
% ============================================================
% AI 声明位置、措辞及配套文件只执行 OFFICIAL-CUMCM-001。
% ============================================================
\clearpage
\section*{AI 工具使用声明}
\ifusedai
...
% ============================================================
% 附录分页执行 PW-FMT-001；内容与支撑材料执行论文写作规范和 OFFICIAL-CUMCM-001。
% ============================================================
\clearpage
...
\begin{longtable}{>{\centering\arraybackslash}p{0.08\textwidth}p{0.34\textwidth}p{0.46\textwidth}}
  ...
  3 & \texttt{data/processed-data.csv} & \TemplateField{按 OFFICIAL-CUMCM-001 填写数据文件及用途} \\
  \ifusedai
...
\ifusedprogram
% 源程序范围与完整性只执行论文写作规范和 OFFICIAL-CUMCM-001。
