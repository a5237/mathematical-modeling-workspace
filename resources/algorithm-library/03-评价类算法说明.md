...
> 定位：本文件是候选算法与实现参考。使用前必须结合当前问题核对指标含义、方向、赋权依据、假设和稳定性验证；示例代码须改写并真实运行，文献条目须独立检索核验后才能引用。
...
def fuzzy_comprehensive_evaluation(weights, membership_matrix, grade_scores=None):
    """
    模糊综合评价（加权平均型）
    :param weights: 指标权重向量，和为 1，shape (m,)
    :param membership_matrix: 隶属度矩阵 shape (m, n)，行=指标，列=评语等级
    :param grade_scores: 各等级赋分 shape (n,)，用于加权评分（可选）
    :return: 综合评语向量 B, 等级（最大隶属度）, 综合得分（可选）
    """
    ...
    B = weights @ R  # 加权平均型合成
    grade = int(np.argmax(B))  # 最大隶属度
    score = None
    if grade_scores is not None:
        score = float(B @ np.array(grade_scores, dtype=float))
    return B, grade, score
...
# 端到端示例：3 指标、4 等级（优/良/中/差）
if __name__ == '__main__':
    # 专家打分隶属度：指标1 有 6/10 专家评为优，等
    W = [0.4, 0.3, 0.3]  # 指标权重（如 AHP 得到）
    R = [
        [0.6, 0.3, 0.1, 0.0],  # 指标1
        [0.3, 0.5, 0.2, 0.0],  # 指标2
        [0.2, 0.4, 0.3, 0.1],  # 指标3
    ]
    grades = [95, 85, 70, 50]  # 各等级赋分
    B, grade, score = fuzzy_comprehensive_evaluation(W, R, grades)
    print(f"综合评语向量 B = {np.round(B, 3)}")
    print(f"最大隶属度等级 = {grade}（优=0/良=1/中=2/差=3）")
